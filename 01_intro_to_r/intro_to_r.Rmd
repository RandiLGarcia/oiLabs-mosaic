...
is not that useful. Instead we can see the top 6 cases with the follwing command.
Type the following line under `data(arbuthnot)` in your lab report by clicking `Enter`
...
that year, respectively. Each case is a year in this dataset. Use the scrollbar 
...
You can see the dimensions (number of rows, the cases, and number of columns, the variables) of this data frame as well as the names of the variables and the first few observations by typing it under `head(arbuthnot)` and then running it:
...
in the chunk (gray part) under `### Exercise 1:` in your lab report. Type your
...
1. What command would you use to extract just the counts of girls baptized? Try
    it! How many girls were baptized in the last year in the dataset?
...
side of the printout to indicate locations within the vector. For example, 4683 
follows [1], indicating that 4683 is the first entry in the vector. And if [43] 
...
What you will see are 82 numbers (in that packed display, because we are not 
looking at a data frame here), each one representing the sum we are after. Take a
...
<div id="boxedtext">
**Tip: ** In the console, if you use the up and down arrow keys, you can scroll through your 
...
```{r load-present-data}
# Type this into the exercise 4 chunk and begin the rest of the exercises
data(present)
```

The data are stored in a data frame called `present`.

4.  What years are included in this data set? *Hint:* Try `max(present$year)` to find the last 
year instead of scrolling through the data. What are the dimensions (rows --- cases --- observations, columns --- variables) of the data frame? What are the variable (column) names? *Hint:* Try the `glimpse()` function, if you get an error, make sure you've run `library(dplyr)`. 

5.  How do these counts of girls and boys compare to Arbuthnot's? Are they of a similar magnitude? *Hint:* Perhaps run `glimpse(arbuthnot)` again in your console to compare.

6.  Make a plot that displays the proportion of boys born over time for the `present` data. What do you see? 
    ...
    able to reuse your code from Ex 3 above, just replace the dataframe name. *Tip:* After updating all of
    that Ex 3 code, you can click the little green arrow at the top of the chunk to run that whole chunk. 

7.  In what year did we see the most total number of births in the U.S.? *Hint:*
    There are two ways to do this 1) use that `max()` function from Ex 4, `max(present$total)`, or 2) you can 
