...
be used throughout the labs. For this data set, each case, or *observation*, is a single flight.
...
1.  Mutate the data frame so that it includes a new variable that contains the 
    ...
    **Hint:** Average speed can be calculated as distance divided by
    number of hours of travel, and note that `air_time` is given in minutes but we want miles per hour.
...
1.  Replicate the following plot. **Hint:** The data frame plotted only
    ...
    Airlines, and the points are `color`ed by `carrier`. 
