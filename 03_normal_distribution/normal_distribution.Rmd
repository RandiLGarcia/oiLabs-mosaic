...
We'll be working with women's heights, which we have already stored as a separate object called `fdims`. Let's calculate some statistics for the women's heights that will be referenced later. 
...
Next we make a density histogram to use as the backdrop and use the `stat_function` 
...
histogram created by the commands below. To make a density histogram instead of
a frequency histogram we add `aes(y = ..density..)` as an argument to the `geom_histogram()` function.

```{r hist-height}
ggplot(data = fdims, aes(x = hgt)) + 
  geom_histogram(aes(y = ..density..)) +
  stat_function(fun = dnorm, args = c(mean = fhgtmean, sd = fhgtsd), col = "springgreen")
```

After initializing a blank plot with the first `ggplot()` command, the `ggplot2` package
allows us to add additional layers. The first layer is a density histogram. The
second layer is a statistical function -- the density of the normal curve, `fun = dnorm`.
...
the line to be drawn. If we left it out, the line would be drawn in black. Try picking a new color, you can check out your options [here](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf)
...
```{r qq}
ggplot(data = fdims, aes(sample = hgt)) +
  geom_qq()
```
...
take one minus that probability *or* add `lower.tail = FALSE` as an argument in the `pnorm()` function.
