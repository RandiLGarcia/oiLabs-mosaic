...
companion package for OpenIntro labs, `oilabs`. We will also use the `tally()` fucntion in `mosaic`.
...
We can then take a look at the distribution of these streak lengths. We'll use a barplot instead of a histogram because streak length is a discrete random variable, not continuous.

```{r plot-streak-kobe}
ggplot(data = kobe_streak, aes(length)) +
  geom_bar()
```
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    you *Knit* it, you should also "set a seed" **before** you sample. 
    
<div id="boxedtext">
Recall that you can set a seed like this:
```{r set-seed}
set.seed(35797)       
```
Remember to do this **before** you sample in the exercise above. Look at your knitted version to see the tally of heads and tails.
</div>
...
6.  Describe the distribution of simulated streak lengths. What is the typical streak 
