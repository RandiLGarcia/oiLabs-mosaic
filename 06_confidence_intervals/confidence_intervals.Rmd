...
in this lab, which means you should set a seed on top of your lab. If you need a refresher, review the lab concerning sampling distributions.
</div>
...
1.  Describe the distribution of house area in your sample. What would you say is the
    ...
    "typical" to mean. Would you expect another student's distribution to be identical to yours? 
...
A confidence interval (CI) takes the following form
\[ \text{point estimate} \pm z_{\alpha/2}^\star \cdot SE \]
Filling in $\bar{x}$ for our point estimate and $\frac{s}{\sqrt{n}}$ for the standard error of the mean, we get the CI for a population mean as 
\[ \bar{x} \pm z_{\alpha/2}^\star \cdot \frac{s}{\sqrt{n}} \]
...
z_star_95
```
which is roughly equal to 1.96. $\pm 1.96$ cut off the rejection regions in hypothesis tests assuming a normal sampling distribution and $\alpha = .05$. This number, $\pm 1.96$, will soon be very familiar to you!
...
To recap: even though we don't know what the full population looks like, we now have a sense of where the true proportion might fall. There are a few conditions that must be met for this interval to be valid. For the confidence interval to be valid, the sample mean must be normally distributed and have standard error $s / \sqrt{n}$.

1.  Interpret the confidence interval in the context of the problem. What does "95% confidence" mean?
...
we can determine if our confidence intervals actually capture it. We'll store it in an object called `mu` (because we call the population mean $\mu$). 

```{r pop-mean}
mu <- mean(~area, data = ames)
```
...
We can accomplish this using the `rep_sample_n` function. The following lines of code takes 50 random samples of size `n` from the population (and remember we defined $n = 60$ earlier), and computes the upper and lower bounds of the confidence intervals based on these samples.
...
Next we'll create a plot similar to Figure 2.37 on page 104 of [the textbook](https://www.openintro.org/os). The first step will be to create a new variable in 
...
```{r plot-ci}
ggplot(data = ci, aes(x = replicate, y = x_bar, color = capture_mu)) +
