...
As you take a moment to review the report, address the following questions.
...
1.  Using the command above, create a new dataframe called `us12` that contains
    ...
    responses. Does it agree with the percentage in Table 6? If not, why? Hint: See the next code chunk in the lab for ideas.
...
being atheists" is answered with a point or interval estimate of the parameter.

The tools for making inferences about population parameters are the confidence interval and the hypothesis test.
...
the standard error and construct the interval in R. Note that since the goal is to construct an interval estimate for a 
...
<!-- error (`me`) associated with each of these values of `p` using the familiar formula ($ME = 2 \times SE$). -->
...
inputPanel(
  numericInput("n", label = "Sample size:", value = 50),
...
1.  Describe the sampling distribution of sample proportions at $n = 50$ and 
    $p = 0.2$. Be sure to note the center, spread, and shape.

1.  Keep $n$ constant and change $p$. Be sure to try $p$'s that would cause us to violate the success-failure condition. How does the shape, center, and spread 
